...
def create(request):
    if request.method == "POST":
        ...
        Account.objects.create(name = name , DOB = dob, Aadhar = aadhar, pan = pan,
                                mobile = mobile , email = email, address = address   )
        #print("Successful")
        send_mail(f"hello {name},thank you for Creating an Account in our Bank ", 
        # subject
...
def balance(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # msg = "PLZ Enter the valid Account Number"
            pass
        if data is not None:
            if data.pin == int(pin):
                bal = data.bal
...
def transfer(request):
    msg = ""
    if request.method =="POST":
        f_acc = request.POST.get('acc1')
        t_acc = request.POST.get('acc2')
        ...
        try:
            ...
            msg = "senders account  number is not valid"
            print(msg)
        try:
            to_acc = Account.objects.get(acc = t_acc)
        except:
            msg = "receiver account is not valid"
        ...
        if from_acc.pin == int(pin):
            if int(amt) > 100 and int(amt) <= 10000 and int(amt) <= from_acc.bal:
                ...
                f"debited to {to_acc.acc} Account Balance is {from_acc.bal} \n ,"
                "regards \n manager(DJD-E1) \n we scam bcz we care " 
                 # body 
                ...
                f"FBH Fraud Bank Hydrabad ,\n from ur {to_acc.acc}  \n , {amt} has been" 
                f"credited from {from_acc.acc} Account Balance is {to_acc.bal} \n ,"
                "regards \n manager(DJD-E1) \n we scam bcz we care " 
                 # body 
